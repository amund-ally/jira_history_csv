...
OUTPUT_FILE = f"jira_history_{FILTER_ID}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
def connect_to_jira(config):
    ...
    return JIRA(
        ...
        basic_auth=(config['jira_email'], config['jira_api_key'])
    )

def get_issue_history(jira, issue):
...
def main():
    try:
        ...
        issues = jira.search_issues(f'filter={FILTER_ID}', maxResults=False)
        ...
        for issue in issues:
            ...
            histories = get_issue_history(jira, issue)
            all_histories.extend(histories)
        
        # Write to CSV
        if all_histories:
            fieldnames = ['issue_key', 'summary', 'from_status', 'to_status', 
            ...
            print(f"Successfully exported {len(all_histories)} status changes to {OUTPUT_FILE}")
        else:
